...
const PhuongTienHuHong = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const { data } = await phuongtienhuhongApi.getList({
          size: 500,
          hu_hong: true,
        });
  ...
  const danhSachDonVi = useMemo(
    () =>
      [...new Set(items.map((i) => i.don_vi_quan_ly))].filter(Boolean).sort(),
    [items],
  );
  const danhSachKetQua = useMemo(
    () =>
      [...new Set(items.map((i) => i.ket_qua || "Chưa xác định"))]
        .filter(Boolean)
        .sort(),
    [items],
  );
  ...
  const filteredItems = useMemo(() => {
    return items.filter((item) => {
      ...
      const matchDonVi =
        ...
        selectedDonVi.includes(item.don_vi_quan_ly);

      const itemKQ = item.ket_qua || "Chưa xác định";
      const matchKetQua =
        selectedKetQua.length === 0 || selectedKetQua.includes(itemKQ);
  ...
  const totalKinhPhi = useMemo(() => {
    return filteredItems.reduce(
      (sum, item) => sum + (parseFloat(item.du_tru_kinh_phi) || 0),
      0,
    );
  }, [filteredItems]);
        ...
        >
          ...
          <Autocomplete
            ...
            renderInput={(params) => (
              <TextField
                {...params}
                label="Lọc theo Hiện trạng/Kết quả"
                size="small"
              />
            )}
            renderTags={(tagValue, getTagProps) =>
              tagValue.map((option, index) => (
                <Chip
                  label={
                    option.length > 20 ? option.slice(0, 14) + ".." : option
                  }
                  {...getTagProps({ index })}
                  color="primary"
                  variant="outlined"
            ...
            >
              ...
              <TableBody>
                {filteredItems.map((item, index) => {
                  const isFinished =
                    (item.ket_qua || "").includes("Đã hoàn thành") ||
                    (item.ket_qua || "").includes("Đã sửa xong");
                  return (
                    <TableRow
                      key={item.id || index}
                      sx={{
                    ...
                    >
                      ...
                      <TableCell className="text-center">
                        {item.nhan_hieu_sat_xi}
                      </TableCell>
                      <TableCell className="text-center fw-bold text-primary">
                        {item.bien_kiem_soat}
                      </TableCell>
                      <TableCell className="text-center">
                        {item.nguoi_quan_ly}
                      ...
                      <TableCell className="text-justify">
                        {item.nguyen_nhan_hu_hong}
                      </TableCell>
                      <TableCell className="text-justify">
                        {item.bien_phap_thuc_hien}
                      </TableCell>
                      <TableCell className="text-justify">
                        {item.de_xuat}
                      </TableCell>
                      <TableCell className="text-right fw-bold">
                        {item.du_tru_kinh_phi
                          ? Number(item.du_tru_kinh_phi).toLocaleString("vi-VN")
                          : "0"}
                      </TableCell>
                      <TableCell align="center">
                        <span
                          className={`badge-status ${isFinished ? "bg-success text-white" : "bg-warning text-dark"}`}
                          style={{
                            padding: "4px 10px",
                            borderRadius: "6px",
                            fontSize: "11px",
          ...
          {filteredItems.length > 0 && (
            ...
            >
              <Typography sx={{ fontWeight: 600, color: "#64748b" }}>
                ...
                >
                  {totalKinhPhi.toLocaleString("vi-VN")} VNĐ
                </span>
              </Typography>
              <Typography
                sx={{ mt: 0.5, fontStyle: "italic", color: "#94a3b8" }}
              >
