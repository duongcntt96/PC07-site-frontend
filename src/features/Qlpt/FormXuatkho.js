...
const FormXuatkho = () => {
  ...
  const onSubmit = async (value) => {
    ...
    const result = await qlptApi.addPhieunhap({
      ...value,
      phuong_tiens: value.phuong_tiens.filter((e) => e.so_luong),
      success: true,
    });
    setID(result.id);
  ...
  return (
    <Stack component="form"
      ...
      <Stack direction="row" spacing={2}>
            ...
            render={({ field }) => (
              <Autocomplete
                ...
                getOptionLabel={(option) => {
                  return (
                    option?.ten ||
                    kho.find((e) => e.id == option)?.ten ||
                    "null"
                  );
                }}
                isOptionEqualToValue={(option, value) => option.id == value}
                onChange={(event, value, reason, details) =>
                  setValue("kho_xuat", value ? parseInt(value.id) : null)
                }
                renderInput={(params) => (
                  <TextField
                    {...params}
        ...
        <Stack width="50%" spacing={2}>
                ...
                renderInput={(params) => (
                  <TextField
                    ...
                    inputProps={{
                      ...params.inputProps,
                      style: { fontSize: "13px" },
                      placeholder: "Để trống trong trường hợp khai báo giảm",
                    }}
                    disabled={isSubmitting}
                  />
                )}
          ...
          <TextField
            ...
            helperText={errors.thoi_gian?.message}
            disabled={isSubmitting}
          />
        </Stack>
      </Stack>
      ...
      {nnn ? (
        ...
        <Stack alignItems="center">
          <Alert severity="warning">{`Vui lòng chọn kho xuất !`}</Alert>
        </Stack>
      )}
      <Button
        variant="contained"
