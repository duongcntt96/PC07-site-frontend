        ...
        <Controller
          ...
          render={({ field }) => (
            <Autocomplete
              ...
              componentsProps={{
                popper: {
                  style: { width: "fit-content", maxWidth: "min(90vw, 500px)" },
          ...
          render={({ field }) => (
            <Autocomplete
              ...
              getOptionLabel={(option) => {
                return (
                  option?.ten ||
                  memoizedChungloaiOptions.find((e) => e.id == option)?.ten ||
                  "null"
                );
              }}
              isOptionEqualToValue={(option, value) => option.id == value}
              onChange={(event, value, reason, details) =>
                setValue("chung_loai", value ? parseInt(value.id) : null)
              }
              renderInput={(params) => (
      ...
      {loading ? (
        ...
        <Stack sx={{ mt: 1 }}>
          <Alert severity="success">
            {`Tìm thấy ${thucluc?.count || 0} danh mục với số lượng ${
              thucluc?.sum || 0
            } !`}
          </Alert>
                  ...
                  .map(({ id, level, ten, maso }) => {
                    if (
                      ...
                      const subtotals = thucluc.data
                        .filter((e) => e.chung_loai__maso.includes(maso))
                        .reduce((sum, value) => (sum += value.totals), 0);
                      if (subtotals)
                        return (
                          <>
                            <TableRow>
                            ...
                            {thucluc.data
                              .filter((e) => e.chung_loai == id && e.totals > 0)
                              ?.map((f) => (
                                <TableRow>
                                  <TableCell
                                    sx={{ pl: `${(level + 1) * 10}px` }}
